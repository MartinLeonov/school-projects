...
**/
	...
	if ((TmpRootPtr) == NULL) {
		...
		do {
			TmpPtr = (TmpRootPtr);
			if (Content < (TmpRootPtr)->Cont) {
				// dale hledam v leve vetvi
				TmpRootPtr = (TmpRootPtr)->LPtr;
			} else if (Content > (TmpRootPtr)->Cont) {
				// dale hledam v prave vetvi
				TmpRootPtr = (TmpRootPtr)->RPtr;
			} else {
				found = true;
			}
		} while (found == false && TmpRootPtr != NULL);
	}

	// vkladani
	if (found) {
		// prepsani starych dat
		TmpPtr->Cont = Content;
	} else {
		// bude vkladany novy prvek
		tBTNodePtr BTItem = malloc(sizeof(struct tBTNode));
		if (TmpPtr == NULL) {
			// prazdny strom, novy uzel bude koren
			*RootPtr = BTItem;
			...
			if (Content < TmpPtr->Cont) {
				// novy uzel se pripoji vlevo
				TmpPtr->LPtr = BTItem;
				BTItem->Cont = Content;
				BTItem->LPtr = NULL;
				BTItem->RPtr = NULL;
			} else {
				// novy uzel se pripoji vpravo
				TmpPtr->RPtr = BTItem;
				BTItem->Cont = Content;
				BTItem->LPtr = NULL;
				BTItem->RPtr = NULL;
...
**/
	...
	while (ptr != NULL) {
		...
		BTWorkOut(ptr);     // zavolani pri zpracovani uzlu
		ptr = ptr->LPtr; // posun
	}
}
...
**/
	...
	tStackP stack;
	SInitP(&stack); // inicializace seznamu
...
**/
	...
	while (ptr != NULL) {
		SPushP(Stack, ptr); // ulozeni ukazatelu do zasobniku // UkNaNejlev :=UkKor;
		ptr = ptr->LPtr;
	}
...
**/
	...
	while (ptr != NULL) {
		SPushP(StackP, ptr); // ulozeni ukazatele
		ptr = ptr->LPtr; // posunuti
		SPushB(StackB, true); // uzel byl navstiven (vlozeni priznaku "zleva")
	}
}
