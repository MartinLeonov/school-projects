...
*/
	...
	if (s == NULL) {
		...
		// Praznemu zasobniku odpovida hodnota top == -1
		s->top = -1;
	}
}
...
*/
	...
	// Ternarni operator; pokud je top == -1 (= prazdny), vrati
	// se hodnota 1 (nenulova hodnota), pokud ne, vrati se 0
	return (s->top == -1) ? 1 : 0;
}
...
*/
	...
	// Ternarni operator; pokud je top == STACK_SIZE-1 (= plny), vrati
	// se hodnota 1 (nenulova hodnota), pokud ne, vrati se 0
...
*/
	...
	// Pokud zasobnik neni prazndy, odstranim prvek z vrcholu (dekremetuji top)
	if (!stackEmpty(s)) {
		s->top--;
	}
}
